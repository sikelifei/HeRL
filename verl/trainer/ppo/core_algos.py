...
PolicyLossFn = Callable[
    [
        ...
        torch.Tensor | None,  # he_mask
        torch.Tensor | None,  # he_log_probs
    ],
...
def compute_policy_loss_vanilla(
    ...
    he_log_probs: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute the clipped policy objective and related metrics for PPO.
...
@register_policy_loss("mixpolicy_vanilla")  # type: ignore[arg-type]
def compute_policy_loss_mixpolicy_vanilla(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    off_mask = he_mask.to(response_mask.device)
    if off_mask.dim() == 1:
        off_mask = off_mask.unsqueeze(-1)
    off_mask = (off_mask > 0).to(response_mask.dtype)
    off_mask = off_mask * response_mask
    on_mask = response_mask * (1 - off_mask)
    ...
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), on_mask)

    pg_losses3 = -advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = verl_F.masked_mean(
        torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float(), on_mask
    )

    on_pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    if rollout_is_weights is not None:
        on_pg_losses = on_pg_losses * rollout_is_weights
    on_pg_losses = on_pg_losses * on_mask

    shaping_gamma = config.policy_loss.get("he_shaping_gamma", 1)
    off_log_ratio = log_prob
    if he_log_probs is not None:
        off_log_ratio = log_prob - he_log_probs.to(log_prob.device)
    off_log_ratio = torch.clamp(off_log_ratio, min=-20.0, max=20.0)
    off_ratio = torch.exp(off_log_ratio)
    if shaping_gamma is None or shaping_gamma <= 0:
        off_weight = off_ratio
    else:
        off_weight = off_ratio / (off_ratio + shaping_gamma)
    off_pg_losses = -advantages * off_weight
    off_pg_losses = off_pg_losses * off_mask

    pg_losses = on_pg_losses + off_pg_losses
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

    on_pg_loss = agg_loss(loss_mat=on_pg_losses, loss_mask=on_mask, loss_agg_mode=loss_agg_mode)
    off_pg_loss = agg_loss(loss_mat=off_pg_losses, loss_mask=off_mask, loss_agg_mode=loss_agg_mode)
    he_frac = verl_F.masked_mean(off_mask, response_mask)
    ppo_kl_on = verl_F.masked_mean(-negative_approx_kl, on_mask)

    pg_metrics = {
        "actor/pg_clipfrac": pg_clipfrac.detach().item(),
        "actor/ppo_kl": ppo_kl_on.detach().item(),
        "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().item(),
        "actor/on_pg_loss": on_pg_loss.detach().item(),
        "actor/off_pg_loss": off_pg_loss.detach().item(),
        "actor/he_frac": he_frac.detach().item(),
    }
    return pg_loss, pg_metrics
