...
from verl.utils.reward_score.if_score.ifbench import instructions_registry
...
def test_ifbench_strict(
    resp,
    ground_truth,
):
  """Tests response to see if instrutions are followed."""
  instruction_list = ground_truth["instruction_id_list"]
  is_following_list = []

  for index, instruction_id in enumerate(instruction_list):
    instruction_cls = instructions_registry.INSTRUCTION_DICT[instruction_id]
    instruction = instruction_cls(instruction_id)
    ground_truth["kwargs"][index] = {key: value for key, value in ground_truth["kwargs"][index].items() if value is not None}
    instruction.build_description(**ground_truth["kwargs"][index])
    args = instruction.get_instruction_args()
    if args and "prompt" in args:
      raise NotImplementedError("Prompt is not supported in this mode")
      # instruction.build_description(prompt=inp.prompt)
  ...
  sparse_score = all(is_following_list)
  dense_score = sum(is_following_list) / len(is_following_list)
  return sparse_score, dense_score, is_following_list
